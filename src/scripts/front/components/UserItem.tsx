...
export const UserItem = ({
  ...
  const [showDeleteWindow, setShowDeleteWindow] =
    useState<boolean>(false)
  ...
  const getTasksCountForUser = (user: TUser, status: string) => {
    switch (status) {
      case "all":
        return tasks.filter(
          (task) =>
            task.asignee._id === user._id && task.status !== "deleted"
        ).length
      case "todo":
        return tasks.filter(
          (task) =>
            task.asignee._id === user._id &&
            task.status !== "deleted" &&
            task.status === status
        ).length
      case "active":
  ...
  return (
    <div key={user._id} id={user.login} className="selected-user">
      <ul className="details-list">
        <li className="details-list__item">
          Zadania użytkownika: {getTasksCountForUser(user, "all")}
        </li>
        <li className="details-list__item">
          Do zrobienia: {getTasksCountForUser(user, "todo")}
        </li>
        <li className="details-list__item">
          W trakcie: {getTasksCountForUser(user, "active")}
        </li>
        <li className="details-list__item">
          Zrobione: {getTasksCountForUser(user, "done")}
        </li>
        <li className="details-list__item">
          Przeterminowane: {getTasksCountForUser(user, "expired")}
        </li>
      </ul>
      <div className="user-role">
        Uprawnienia:{" "}
        {user.role === "user" ? "użytkownik" : "administrator"}
      </div>
      ...
      {showDeleteWindow && (
        <DeleteUserWindow
          ...
          handleDeleteAccount={() => deleteUserFromDatabase(user)}
          user={user}
        />
      )}
    </div>
